...
AdsDevice::AdsDevice() : m_NetId(AddRoute(defaultNetID, defaultIPV4.c_str()), {[](AmsNetId ams)
                                                                               {bhf::ads::DelLocalRoute(ams); return 0; }}),
                         m_Addr({defaultNetID, defaultPort}),
                         m_LocalPort(new long{AdsPortOpenEx()}, {AdsPortCloseEx})
{
}

AdsDevice::AdsDevice(const std::string &ipV4, AmsNetId netId, uint16_t port)
    : m_NetId(AddRoute(netId, ipV4.c_str()), {[](AmsNetId ams)
                                              {bhf::ads::DelLocalRoute(ams); return 0; }}),
      m_Addr({netId, port}),
      m_LocalPort(new long{AdsPortOpenEx()}, {AdsPortCloseEx})
{
...
{
    return {new uint32_t{offset}, {[](uint32_t)
                                   { return 0; }}};
}
